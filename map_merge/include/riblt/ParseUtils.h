...
{
    if (v.size() != sizeof(T))
        throw std::runtime_error("from_sv: incorrect size");
    T ret;
    std::memcpy(&ret, v.data(), sizeof(T));
    return ret;
}
...
{
    if (encoded.size() < 1)
        throw std::runtime_error("parse ends prematurely");
...
inline uint64_t decodeVarInt(std::string_view &encoded)
{
    ...
    {
        if (encoded.size() == 0)
            throw std::runtime_error("premature end of varint");
